...
async def trace():
    from runtime.step_executor import RuntimeState, _compute_feasibility
    ...
    state = RuntimeState()
    feasibility_rules = build_feasibility_rules(parsed)
    ...
    for step in plan.steps:
        ...
        if step.step == "detect":
            ...
            state.artifacts["detections"] = detections
            for idx, det in enumerate(detections):
                ...
                c = Candidate(object_id=f"obj_{idx}", detector_score=det.score, bbox=bbox,
                    crop_path=det.crop_path, analysis_path=analysis_path, analysis_bbox=analysis_bbox)
                state.candidates.append(c)
            for c in state.candidates:
                print(f"  => {c.object_id}: score={c.detector_score:.2f}  bbox=({c.bbox.x1:.0f},{c.bbox.y1:.0f},{c.bbox.x2:.0f},{c.bbox.y2:.0f})  state={c.state.value}")
            print()

        elif step.step == "nms":
            before = sum(1 for c in state.candidates if c.state is CandidateState.DETECTED)
            apply_nms(state.candidates, iou_threshold=0.5)
            after = sum(1 for c in state.candidates if c.state is CandidateState.DETECTED)
            for c in state.candidates:
                tag = "SUPPRESSED" if c.state is CandidateState.SUPPRESSED else "KEPT"
                info = f"  => {c.object_id}: {tag} (state={c.state.value})"
                print(info)
            print()
            ...
            for c in state.candidates:
                ...
                if ver.get("ok") is False:
                    c.transition_to(CandidateState.REJECTED, "verify", "verification rejected")
                    print(f"  => {c.object_id}: REJECTED  score={c.verify_score:.2f}")
                else:
                    c.transition_to(CandidateState.VERIFIED, "verify", "verification confirmed")
            ...
            for c in state.candidates:
                if c.is_suppressed_or_rejected:
                    print(f"  => {c.object_id}: SKIPPED (state={c.state.value})")
                    continue
                ...
                _compute_feasibility(c, feasibility_rules)
            ...
            merge_result = await executor._merger.merge(
                ...
                scene_pure_negative=state.scene_flags.get("pure_negative", False), run_id="trace", execution_log_text="")
    ...
    print("=" * 70)
    for c in state.candidates:
        print(f"  {c.object_id}: state={c.state.value}  det_score={c.detector_score:.2f}  verify_score={c.verify_score:.2f}  confidence={c.confidence:.3f}")
        print(f"    history:")
        for h in c.history:
            print(f"      [{h['step']}] {h.get('reason') or h.get('message') or h.get('decision', '?')}")
